...
# Flask (Web-UI), Gunicorn (Prod-Server), Meshtastic-CLI
$PIP install flask gunicorn meshtastic

echo "==> Schreibe settings.json (Standard: Channel 2, Europe/Berlin)"
...
echo "==> Schreibe web.py (Dark Mode + 30-Minuten-Fenster)"
...
echo "==> Schreibe dispatch.py (sendet fällige Einträge via Meshtastic-CLI)"
...
echo "==> Füge Benutzer zur 'dialout'-Gruppe hinzu (Seriellzugriff)"
sudo usermod -aG dialout "$USER_NAME" || true

echo "==> Schreibe systemd-Services & Timer (Web: 0.0.0.0:8080; Dispatcher: alle 30 Min)"
...
echo "  - settings.json  (Default: ch-index 2)"
echo "  - reminders.json"
echo " Dienste: mesh-reminders-web.service"
echo " Timer:   mesh-reminders-dispatch.timer (alle 30 Min)"
echo
echo " Logs:"
echo "  sudo journalctl -u mesh-reminders-web -f"
echo "  sudo journalctl -u mesh-reminders-dispatch -f"
echo
echo " HINWEIS: Ab-/Anmelden, damit Dialout-Gruppenzugehörigkeit aktiv wird."
echo "=========================================================="
